"""Crea tablas REALTIME de Fase 3 comercial (locale + impuestos)."""
import json
import os
import sys
import time
...
TABLES = ["fact_user_locale", "fact_order_taxes"]
...
if __name__ == "__main__":
    print("=" * 55)
    print("  GameMetrics — Locale + impuestos (Fase 3 comercial)")
    print("=" * 55)
    for _ in range(20):
        try:
    ...
    failed = [n for n in TABLES if not setup_table(n)]
    if failed:
        print(f"ERROR: {failed}")
        sys.exit(1)
    print(f"\nOK: {len(TABLES)} tablas Fase 3 comercial")
